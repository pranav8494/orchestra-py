"""Tests for the Analytics agent's subprocess Python executor.

These really do start interpreters: the thing under test *is* the process boundary, and a
mocked `create_subprocess_exec` would prove nothing about the kill, the scrubbed
environment or the network guard. Every script is stdlib-only and the timeout test injects
its own clock, so the suite stays fast.

The two tests that mock do so *at* the boundary — there is no way to run a real process out
of descriptors on purpose.
"""
...
# machine, far below the 15 s default.
SHORT_TIMEOUT = 1.0

# Both halves matter: the flood is what used to leave the read transport paused, and the
# loop is what used to make the reap after the kill wait for a pipe disconnect that could
# no longer happen.
...
def wait_for(path: Path) -> bool:
    ...
    Polled because the signal crosses a process boundary, where an `asyncio.Event` does not
    reach; in a thread so the loop stays free to run the child's transport.
    """
...
def wait_until_gone(pid: int) -> bool:
    ...
    Signal 0 is the liveness question without the side effect. Polled rather than waited on
    because a grandchild is not this process's to reap: once the group dies it is init's.
    """
    deadline = time.monotonic() + TIMEOUT
    while time.monotonic() < deadline:
        try:
...
def test_info_advertises_its_params_schema(tool: RunPythonTool) -> None:
    """The published shape: this crosses to the model."""
    spec = tool.info()
...
async def test_run_python_returns_what_the_script_printed(tool: RunPythonTool) -> None:
    response = await tool.run(tool_call(TOOL_NAME, code="print(6 * 7)"))
...
async def test_run_python_writes_each_input_into_the_working_directory(
    ...
    """The description's promise: `artifact:sales.csv` is readable as `./sales.csv`."""
...
async def test_run_python_bad_pointer_returns_an_error_not_an_exception(
    ...
    response = await tool.run(tool_call(TOOL_NAME, code="print(1)", inputs=[pointer]))
...
async def test_run_python_failing_script_returns_its_own_traceback(tool: RunPythonTool) -> None:
    """`run_path` in the child is what names the model's file and line, not ours."""
    response = await tool.run(tool_call(TOOL_NAME, code="total = 1 / 0\n"))
...
async def test_run_python_infinite_loop_is_killed_at_the_timeout(store: ArtifactStore) -> None:
    ...
    response = await asyncio.wait_for(
        tool.run(tool_call(TOOL_NAME, code="while True:\n    pass\n")), timeout=TIMEOUT
    )
...
async def test_run_python_infinite_loop_that_floods_stdout_is_stopped(tool: RunPythonTool) -> None:
    """Regression: a script writing faster than the parent reads left the read transport
    paused, and the reap after the kill then waited on a pipe disconnect nothing could
    deliver — the child died, the parent hung. On the *default* clock, so a regression
    fails in seconds instead of quietly returning at the timeout.
    """
    response = await asyncio.wait_for(
        tool.run(tool_call(TOOL_NAME, code=FLOOD_THEN_LOOP)), timeout=TIMEOUT
    )
...
async def test_run_python_output_past_the_cap_is_never_buffered_whole(tool: RunPythonTool) -> None:
    """Regression: reading to EOF cost the parent the script's whole output per concurrent
    subtask — 300 MB printed drove peak RSS up by nearly a gigabyte, and the cap only
    trimmed what was already in memory."""
    code = "import sys\nfor _ in range(200):\n    sys.stdout.write('x' * 100000)\n"

    response = await asyncio.wait_for(tool.run(tool_call(TOOL_NAME, code=code)), timeout=TIMEOUT)
...
async def test_run_python_kills_a_background_process_the_script_left_running(
    ...
    """The group is killed on success too, not only on the timeout and the cancel: a
    grandchild with its own stdio does not hold the pipes open, so the script returns
    cleanly and used to leave it writing into a deleted working directory."""
    code = (
        "import subprocess\nimport sys\n"
...
async def test_run_python_cannot_connect_a_socket(tool: RunPythonTool) -> None:
    ...
    response = await tool.run(
        tool_call(TOOL_NAME, code="import socket\nsocket.socket().connect(('127.0.0.1', 9))\n")
    )
...
async def test_run_python_can_import_urllib_and_still_cannot_reach_anything(
    ...
    """The guard patches the connect methods, not the `socket.socket` class: `ssl` does
    `class SSLSocket(socket)`, so replacing the class with a function broke the next
    `import urllib.request` with a `TypeError` the model could not fix. The address is the
    loopback discard port, so a failed guard refuses a connection rather than making one.
    """
    code = (
    ...
    response = await tool.run(tool_call(TOOL_NAME, code=code))
...
async def test_run_python_spawn_failure_is_returned_as_data(
    ...
    response = await tool.run(tool_call(TOOL_NAME, code="print(1)"))
...
async def test_run_python_staging_failure_is_returned_as_data(
    ...
    """The same for the filesystem: data, not an exception."""
...
async def test_run_python_child_environment_carries_no_parent_secret(
    ...
    response = await tool.run(
        tool_call(TOOL_NAME, code="import os\nprint(os.environ.get('ANTHROPIC_API_KEY'))\n")
    )
...
async def test_run_python_unknown_argument_is_an_error(tool: RunPythonTool) -> None:
    ...
    response = await tool.run(tool_call(TOOL_NAME, code="print(1)", timeout=1))
...
async def test_run_python_missing_code_is_an_error(tool: RunPythonTool) -> None:
    response = await tool.run(tool_call(TOOL_NAME, inputs=[]))
...
async def test_run_python_script_that_prints_nothing_is_empty_not_failed(
    ...
    """The third outcome, so nobody retries a script that worked."""
    response = await tool.run(tool_call(TOOL_NAME, code="total = 1 + 1\n"))
...
async def test_run_python_truncates_output_that_would_flood_the_prompt(tool: RunPythonTool) -> None:
    """This text goes into the next prompt, so the cap is part of the contract."""
    response = await tool.run(tool_call(TOOL_NAME, code=f"print('x' * {MAX_OUTPUT_CHARS * 2})"))
...
async def test_run_python_propagates_cancellation(tool: RunPythonTool, tmp_path: Path) -> None:
    """§10: `CancelledError` is the only thing that may leave `run`. The marker puts the
    cancel inside the reads rather than in staging, so what is under test is the cleanup
    that kills the process group and lets the error through."""
    marker = tmp_path / "child-started"
    code = (
        f"import pathlib, time\npathlib.Path({str(marker)!r}).touch()\ntime.sleep({TIMEOUT * 4})\n"
    )
    task = asyncio.create_task(tool.run(tool_call(TOOL_NAME, code=code)))
