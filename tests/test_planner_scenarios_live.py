"""Live half of the #17 scenario suite — deselected by default (CONVENTIONS.md §12).

```bash
uv run pytest -m live          # needs ANTHROPIC_API_KEY; costs three model calls
```

This is the suite that actually answers "is the planner dynamic?". The offline half
proves the shape survives conversion; only a real model can show that three differently
shaped requests produce three differently shaped plans, and that is the design risk the
implementation plan pulled #17 forward to measure.

A failure here is a prompt problem, not a test problem: the shapes are the research
doc's table, and `prompts/planner.py` is what has to satisfy them.

Config is read at import, before `conftest._isolated_env` cuts the environment off, and
through `load_config()` rather than `os.environ` — §6 keeps that read in one module.
"""
